...
(function () {
  ...
  */
	...
	$$(classes.component)._.events({
		'click': function onOptionListClick(_ref) {
			var target = _ref.target;

			var isChecked = !target.classList.contains(classes.state.selected);
  ...
  */
	...
	$editor._.events({
		'select': function onEditorTextSelect(e) {
			...
			var left = Math.min(currentMousePosition.x - editorOffset.left - Math.abs((currentMousePosition.x - startMousePosition.x) / 2) - 30 * 2, editorRight);
